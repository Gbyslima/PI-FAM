...
if (!empty($_GET['id'])) {
    ...
    if ($result->num_rows > 0) {
      while($user_data = $result->fetch_assoc()) {
        $id = $user_data['id'];
        $paciente = $user_data['paciente'] ?? '';
        $data = $user_data['data'] ?? null;
        $horario = $user_data['hora'] ?? '';
...
<div class="form-container">
  <h2>Editar</h2>
  <form action="agenddamento.php" method="post">
    <input type="hidden" name="id" value="<?php echo htmlspecialchars($id); ?>">

    <input type="text" name="paciente" placeholder="paciente" value="<?php echo htmlspecialchars($paciente); ?>" required>

    <input type="text" name="servico" placeholder="Serviço" value="<?php echo htmlspecialchars($servico); ?>" required>

    <input type="text" name="profissional" placeholder="Profissional" value="<?php echo htmlspecialchars($profissional); ?>" required>

    <input type="date" name="data" id="data" value="<?php echo htmlspecialchars($data); ?>" required>

    <input type="time" name="horario" id="horario" value="<?php echo htmlspecialchars($horario); ?>" required>

    <textarea name="observacao" placeholder="Observação"><?php echo htmlspecialchars($observacao); ?></textarea>

    <button type="submit" name="update">Salvar</button>
    <button type="button" onclick="window.location.href='agenda.php'">Voltar</button>
  </form>
</div>
