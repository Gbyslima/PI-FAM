...
if (!empty($_GET['id'])) {
    ...
    $sqlSelect = "SELECT * FROM profissionais WHERE id=$id";
    ...
    if ($result->num_rows > 0) {
      while($user_data = $result->fetch_assoc()) {
        $id = $user_data['id'];
        $nome = $user_data['nome'] ?? '';
        $data_contratacao = $user_data['data_contratacao'] ?? null;
...
<head>
  <meta charset="UTF-8" />
  <title>Profissional - Editar</title>
...
<div class="form-container">
  <h2>Editar</h2>
  <form action="dbprofissional.php" method="POST">
    ...
    <input type="date" name="data_contratacao" id="data_contratacao" value="<?php echo htmlspecialchars($data_contratacao); ?>" required>

    <input type="email" name="email" placeholder="email" value="<?php echo htmlspecialchars($email); ?>" required>

    <input type="text" name="especialidade" placeholder="especialidade" value="<?php echo htmlspecialchars($especialidade); ?>" required>


    <button type="submit" name="update">Salvar</button>
    <button type="button" onclick="window.location.href='profissional.php'">Voltar</button>
  </form>
</div>
