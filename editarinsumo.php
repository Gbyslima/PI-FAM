...
if (!empty($_GET['id'])) {
    ...
    $sqlSelect = "SELECT * FROM insumo WHERE id=$id";
    ...
    if ($result->num_rows > 0) {
      while($user_data = $result->fetch_assoc()) {
        ...
        $nome = $user_data['nome'] ?? '';
        $tipo = $user_data['tipo'] ?? '';
        $validade = $user_data['validade'] ?? null;
        $status = $user_data['status'];
        $quantidade_estoque = $user_data['quantidade_estoque'];
        $unidade_medida = $user_data['unidade_medida'];
      } 
    } else {
...
<body>
  <aside>
    ...
    <nav>
      ...
      <a href="pacientes.html"><i data-lucide="users"></i>Pacientes</a>
      <a href="agenda.php" class="active"><i data-lucide="calendar-days"></i>Agenda</a>
      <a href="servicos.html"><i data-lucide="list-collapse"></i>Serviços</a>
...
<div class="form-container">
  <h2>Editar</h2>
  <form action="dbinsumos.php" method="POST">
    <input type="hidden" name="id" value="<?php echo htmlspecialchars($id); ?>">

    <input type="text" name="nome" placeholder="nome" value="<?php echo htmlspecialchars($nome); ?>" required>

    <input type="text" name="tipo" placeholder="tipo" value="<?php echo htmlspecialchars($tipo); ?>" required>

    <input type="text" name="status" placeholder="status" value="<?php echo htmlspecialchars($status); ?>" required>

    <input type="date" name="validade" id="validade" value="<?php echo htmlspecialchars($validade); ?>" required>

    <input type="text" name="quantidade_estoque" placeholder="quantidade_estoque" value="<?php echo htmlspecialchars($quantidade_estoque); ?>" required>

    <input type="text" name="unidade_medida" placeholder="unidade_medida" value="<?php echo htmlspecialchars($unidade_medida); ?>" required>


    <button type="submit" name="update">Salvar</button>
    <button type="button" onclick="window.location.href='insumos.php'">Voltar</button>
  </form>
</div>
